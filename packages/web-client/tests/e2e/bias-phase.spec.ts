...
import type { ReplayBiasChoiceEvent, ReplayRecording } from 'app/replay-buffer';
import {
    gotoLuckyBreak,
    drainEvents,
    installEventHarness,
...
test('bias selections persist across rounds and record deterministic replays', async ({ page }) => {
    ...
    type ReplayEvent = ReplayRecording['events'][number];
    const recordedChoices = replay.events.filter(
        (event: ReplayEvent): event is ReplayBiasChoiceEvent => event.type === 'bias-choice',
    );
